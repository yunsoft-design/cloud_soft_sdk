...
class YsModelsManager(models.Manager):
    """
    基础抽象模型管理器
    """
    _page_cnt = 50
    ...
    def sql_browse_lst(self, q=None, record_id=None, page_num=None, field_fun=None, fun_q=None):
        ...
        # 1 生成条件过滤数据
        if q is None:
            objs = super(YsModelsManager, self).get_queryset().all()
        else:
            objs = super(YsModelsManager, self).get_queryset().filter(q)
        # 2 生成分页数据
        ...
        if record_id is not None:
            selected = super(YsModelsManager, self).get_queryset().get(id=record_id)
    ...
    def _filter_q(self, q):
        ...
        """
        if q is None:
            records = super(YsModelsManager, self).get_queryset().all()
        else:
            records = super(YsModelsManager, self).get_queryset().filter(q)
        return records
    ...
    def _get_selected_lst(self, selected_lst, field_fun):
        records = super(YsModelsManager, self).get_queryset().filter(id__in=selected_lst)
        sel_lst = []
        f = operator.methodcaller(field_fun)
        for record in records:
    ...
    def stick(self, record_id, q=None):
        ...
        """
        if q is None:
            curr_record = super(YsModelsManager, self).get_queryset().filter(id=record_id).first()
        else:
            curr_record = super(YsModelsManager, self).get_queryset().filter(q).filter(id=record_id).first()
        if curr_record is not None:  # 如果当前记录存在
            if curr_record.order > 0:  # 1 如果已经置顶了,则取消置顶
                ...
                if q is None:
                    records = super(YsModelsManager, self).get_queryset().filter(order__gt=0)
                else:
                    records = super(YsModelsManager, self).get_queryset().filter(q).filter(order__gt=0)
                print(len(records))
